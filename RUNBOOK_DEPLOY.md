...
**Cierre entrega tras humano (`#final_entrega`):** con sesión admin, `#c` al cliente y luego `#final_entrega` (o `#final_entrega` + nombre/tel/`#n` en una línea; `!!final_entrega` si WhatsApp altera el `#`). Solo **quita** `humanoAtendiendo` en `chats/{jid}` (no borra `silenciadoHasta` del panel), marca `cierreEntregaAsistido`, envía al cliente el texto de `config/prompts.mensajeClienteCierreEntregaHumano` (panel **Instrucciones AI** → pestaña **Cierre entrega**) y Vicky recibe `instruccionCierreEntregaHumanoGemini` hasta que persista `[ENTREGA:…]` en agenda; entonces se limpia el flag. Solo WhatsApp (no `ig:`). **Redeploy** `vicky-bot` y `vicky-dashboard` si cambiás código o UI del panel.
